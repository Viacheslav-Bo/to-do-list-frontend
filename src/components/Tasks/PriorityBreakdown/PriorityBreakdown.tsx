      ...
      : <div className="flex flex-col gap-2 px-1 sm:gap-1.5">
          {PRIORITY_TIERS.map((tier) => {
            ...
            return (
              ...
              >
                <span className="w-20 shrink-0 text-[clamp(0.7rem,1.8vw,0.75rem)] text-[var(--color-text-secondary)] sm:w-16">
                  {tier.label}
                </span>
                <div className="min-w-0 flex-1 h-1.5 overflow-hidden rounded-full bg-[var(--color-surface-solid)]">
                  <div
                    className={`h-full ${tier.color} transition-all duration-500`}
                    style={{ width: `${percent}%` }}
                  />
                </div>
                <span className="w-8 shrink-0 text-right font-mono text-[clamp(0.7rem,1.8vw,0.75rem)] font-medium text-[var(--color-text-muted)] sm:w-6">
                  {count}
                </span>
              </div>
            );
